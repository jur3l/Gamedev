...
- [GPU_SIMULATION.md](GPU_SIMULATION.md) — Phase 2A-2E were **EXPERIMENTAL** (feasibility PoC only). **Phase 2F (Production Wiring) is no longer experimental-only**: GPU now actually computes SAND+WATER *movement* for real gameplay (`Main.tscn`), via small additive changes to `World`/`PixelSimWorld` (§4/§9's `set_cell()`, mining, etc. below are otherwise unmodified — only a new opt-in per-material movement-ownership gate was added). **A real, measured performance regression was found and root-caused, but not yet fixed** — see [SIMULATION_TIMESCALE.md "Regression Investigation"](SIMULATION_TIMESCALE.md#regression-investigation--root-cause-analysis) before assuming this is a net win. Everything else (reactions, GRAVEL/LAVA, mining, building, player collision, rendering) remains exactly as this document describes, CPU-only, unmodified.
- [SIMULATION_TIMESCALE.md](SIMULATION_TIMESCALE.md) — The fixed-timestep/accumulator/GPU-integration layer on top of GPU_SIMULATION.md's Phase 2A/2B solvers. Phase 2C/2D/2E were GPU-path-only and experimental; Phase 2F wires it into production for SAND+WATER movement (see above). The CPU's `simulation_budget_ms`/`step_simulation()` model described in §4/§7 below is unmodified — `World::step()` gained one new gate (skip the movement dispatch for an externally-owned material), nothing else changed.
- [GPU_ACTIVE_REGION.md](GPU_ACTIVE_REGION.md) — **EXPERIMENTAL.** Phase 2D: makes GPU dispatch cost track active workload instead of total world-buffer size, via a GPU-computed activity bounding box and rect-limited dispatch. Builds on SIMULATION_TIMESCALE.md's fixed-timestep backend; the CPU chunk sleeping/activation model (SIMULATION_ACTIVATION.md) is its reference semantics, not something it replaces or modifies.
- [GPU_MATERIAL_INTERACTIONS.md](GPU_MATERIAL_INTERACTIONS.md) — **EXPERIMENTAL.** Phase 2E: a general, material-agnostic GPU reaction framework (dense rule lookup table + mutual-agreement resolution), validated with two synthetic placeholder materials, not Lava. The CPU Material Reaction System (MATERIAL_REACTIONS.md) is its reference semantics; existing movement/interaction code is unmodified.
...
**What PixelSim is not (yet):** a shipped game, a multiplayer system, a save/load system, an infinite/streaming world, or a multithreaded simulation. It is now a *partial* GPU-resident simulation — SAND+WATER *movement* runs on GPU in production (see [GPU_SIMULATION.md "Phase 2F"](GPU_SIMULATION.md#phase-2f--production-wiring-sandwater-movement)) — with a known, measured, not-yet-fixed performance regression; everything else (reactions, GRAVEL/LAVA, terrain, mining, rendering) is still CPU-only, same as always. See [§16 Current Limitations](#16-current-limitations) and [§17 Future Directions](#17-future-directions).
...
- **GPU-resident cellular simulation** (experimental backend alongside the existing CPU reference/production backend) — see [GPU_SIMULATION.md](GPU_SIMULATION.md) (Phase 2A SAND, Phase 2B WATER, Phase 2C real-time timestep integration, Phase 2D active-region dispatch, Phase 2E material interaction/reaction architecture — all DONE), [SIMULATION_TIMESCALE.md](SIMULATION_TIMESCALE.md), [GPU_ACTIVE_REGION.md](GPU_ACTIVE_REGION.md), and [GPU_MATERIAL_INTERACTIONS.md](GPU_MATERIAL_INTERACTIONS.md). Still experimental, not wired into `PixelSimWorld`/`Main.tscn`/any production path, and still SAND+WATER only for real materials (Phase 2E's reaction system is validated with synthetic placeholder materials, not LAVA — no Material Reaction System content, mining, activation/sleeping, or Background/Foreground on GPU) — a production migration remains its own future, unstarted, deliberately-scoped milestone per each phase's own "Architecture Decision" section.
