...
- [GPU_SIMULATION.md](GPU_SIMULATION.md) — **EXPERIMENTAL.** The GPU simulation feasibility PoC (Phase 2A SAND, Phase 2B WATER, Phase 2C real-time timestep integration, Phase 2D active-region dispatch). The CPU simulation described throughout this document remains PRODUCTION/REFERENCE, completely unmodified by it.
- [SIMULATION_TIMESCALE.md](SIMULATION_TIMESCALE.md) — **EXPERIMENTAL.** The fixed-timestep/accumulator/GPU-integration PoC layered on top of GPU_SIMULATION.md's Phase 2A/2B solvers, decoupling physical simulation time from CPU compute workload for the GPU path only. The CPU's `simulation_budget_ms`/`step_simulation()` model described in §4/§7 below is unmodified by it.
