...
- **GPU-resident cellular simulation** (experimental backend alongside the existing CPU reference/production backend) — see [GPU_SIMULATION.md](GPU_SIMULATION.md) (Phase 2A SAND, Phase 2B WATER, Phase 2C real-time timestep integration, Phase 2D active-region dispatch — all DONE), [SIMULATION_TIMESCALE.md](SIMULATION_TIMESCALE.md), and [GPU_ACTIVE_REGION.md](GPU_ACTIVE_REGION.md). Still experimental, not wired into `PixelSimWorld`/`Main.tscn`/any production path, and still SAND+WATER only (no LAVA, Material Reaction System, mining, activation/sleeping, or Background/Foreground on GPU) — a production migration remains its own future, unstarted, deliberately-scoped milestone per SIMULATION_TIMESCALE.md's and GPU_ACTIVE_REGION.md's own "Architecture Decision" sections.
