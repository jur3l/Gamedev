...
All five tiers reached `SETTLED` with `Fresh Scene: yes` and `V-Sync: OFF` confirmed — no timeouts, no invalid tiers. **Two things are visible now that weren't before:** (1) at the light tiers (10k–100k), the achieved FPS (257–337 avg, up to 416 max) genuinely exceeds the 240 Hz display ceiling — proof the earlier V-Sync-capped numbers were an artificial cap, not the real achievable rate; (2) at the heavy tiers (250k–500k), avg FPS settles to ~205, *below* 240 Hz — this is the **real, uncapped performance ceiling** at that load, not a presentation artifact, and is the number that actually matters for a future CPU/GPU comparison. Max frame time (~25–34 ms across every tier) is consistently far above the average — a periodic spike (likely GC/allocation or an occasional heavier chunk-scan pass) worth noting for future investigation, but not addressed here per this milestone's isolation-and-uncapping-only scope.

**Superseded by [Stress Test — Fixed-Timestep Physics](#stress-test--fixed-timestep-physics) below.** The "unmodified physics" claim above turned out to be the whole problem: uncapping V-Sync/FPS in this round made the *benchmark* honest about the render ceiling, but it also meant the underlying physics ran measurably *faster* in wall-clock terms than its true, correctly-paced rate — because on the (buggy, pre-fix) CPU path, more render FPS directly meant more `step_simulation()` calls per second, and each one did real, budgeted simulation work. These "Time to settle" numbers are kept here, unmodified, as the historical record of what render-FPS-coupled physics produced — not a valid cross-tier or future CPU/GPU baseline.

---

## Stress Test — Fixed-Timestep Physics

**Problem confirmed:** `main.gd`'s `_process(delta)` called `sim_world.step_simulation(delta)` once per render frame, unconditionally — and since `step_simulation()` → `World::step()` is itself bounded by real wall-clock CPU time (`simulation_budget_ms`), not simulated time, calling it more often (higher render FPS) meant strictly more physical progress per real second. **RENDER FPS ≠ PHYSICS SPEED is a core architectural invariant this violated** — see SIMULATION_TIMESCALE.md "CPU Fixed Timestep (Production)" for the full writeup, the fix design, and a standalone FPS-independence validation showing byte-exact identical physical state across 60/120/240/500 FPS synthetic render cadences (proof, not assumption).

**Fix:** `CPUSimulationBackend` (`project/scripts/cpu_simulation_backend.gd`) — a fixed-timestep accumulator (`fixed_dt = 1/60s`, the project's existing Phase 2C convention, reused not reinvented) gating calls to the unmodified `PixelSimWorld.step_simulation()`. `simulation_budget_ms` (4.0ms, unchanged) still bounds how much real CPU time a single call may spend; the accumulator now separately bounds how *often* a call happens (~60/sec regardless of render FPS, capped at `max_ticks_per_frame = 10` to prevent runaway catch-up). Zero C++ files changed — `World`/`PixelSimWorld` are byte-for-byte identical to before this fix.

**Measured** (live, fresh scene + same terrain seed per tier, V-Sync OFF, uncapped FPS, run until fully settled — same isolation discipline as the table above, now with correctly-paced physics):

| Tier | Fresh Scene | Sand | Settled | Time to Settle | Physical Sim Time | Ticks | Avg FPS | Min FPS | Max FPS | Avg Sim ms | Peak Active | Peak Backlog |
|---:|:---:|---:|:---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | yes | 10k | yes | 12.29s | 12.30s | 738 | 1707.1 | 240.0 | 1975.0 | 0.783 | 96 | 0 |
| 2 | yes | 50k | yes | 12.49s | 12.50s | 750 | 1325.5 | 240.0 | 1896.0 | 2.225 | 96 | 0 |
| 3 | yes | 100k | yes | 21.94s | 21.95s | 1317 | 1340.9 | 240.0 | 1520.0 | 2.239 | 96 | 0 |
| 4 | yes | 250k | yes | 35.73s | 35.73s | 2144 | 1136.5 | 240.0 | 1365.0 | 3.162 | 144 | 0 |
| 5 | yes | 500k | yes | 52.91s | 52.92s | 3175 | 1057.7 | 240.0 | 1357.0 | 3.698 | 192 | 0 |

**Every single tier: wall-clock "Time to settle" equals physical "simulation time" to within 0.01s, and `peak backlog = 0` throughout.** This is the direct, live-gameplay confirmation of the fix: physics never fell behind real time and never raced ahead of it — it advanced at exactly the fixed rate the accumulator targets, for every tier from 10k to 500k. Compare against the pre-fix table above: 10k went from 1.39s (render-FPS-inflated, ~337 FPS avg) to 12.29s (correctly paced) — a **~9× slowdown**, entirely explained by the fact that the old number was never a valid physics measurement, only a measurement of how fast this machine could call an unthrottled loop. Achieved FPS is now dramatically higher across the board (1057–1707 avg, vs. 205–337 before) precisely *because* simulation work is now capped at ~240ms of real CPU time per second regardless of render speed, freeing up far more frame budget for rendering — visible, measurable proof that render and physics are now genuinely decoupled, not just decoupled in theory.

**Tick count vs. passes completed diverge at the heavier tiers** (100k: 1317 ticks / 740 passes; 500k: 3175 ticks / 604 passes) — expected: as active material grows, a single tick's 4ms budget increasingly can't complete a full bottom-to-top pass in one call, so more ticks are needed per pass. This is `simulation_budget_ms` doing exactly its documented job (bounding per-call CPU time), composing correctly with the new fixed-tick-rate ceiling above it.

**This is now the credible baseline for the next CPU/GPU comparison** — every number reflects genuinely fresh terrain, an uncapped frame rate, and physics that is provably independent of both.
