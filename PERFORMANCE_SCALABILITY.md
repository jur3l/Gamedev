...
**Scope of the fifth milestone — [Stress Test — Frame-Time / Stutter / Simulation-Tick Instrumentation](#stress-test--frame-time--stutter--simulation-tick-instrumentation):** triggered by a direct discrepancy report — the previous milestone's stress-test numbers (10k → ~2100 avg FPS, 500k → ~1127 avg FPS) read as excellent, but manual gameplay at the 500k tier visibly and consistently stutters. This milestone is **instrumentation and methodology only** — it builds the tooling to measure *why* a high average FPS can coexist with visible stutter (frame-time distribution, explicit stutter-frame counts, per-tick simulation timing, frame-vs-simulation split), and explicitly does **not** re-run or re-report the 10k–500k benchmark itself. Per the request's explicit instruction, the actual 10k–500k tier runs for this milestone are a **human-run, human-observed** measurement pass performed manually by the user after this instrumentation shipped — not generated by an agent. No gravity/timestep/solver/budget/GPU-wiring change of any kind was made. See [Stress Test — Frame-Time / Stutter / Simulation-Tick Instrumentation](#stress-test--frame-time--stutter--simulation-tick-instrumentation) below and [SIMULATION_TIMESCALE.md "Stress-Test Instrumentation Trustworthiness"](SIMULATION_TIMESCALE.md#stress-test-instrumentation-trustworthiness-frame-time--stutter--per-tick-timing).
...
**Superseded for trustworthiness purposes by [Stress Test — Frame-Time / Stutter / Simulation-Tick Instrumentation](#stress-test--frame-time--stutter--simulation-tick-instrumentation) below.** The table above's "Avg FPS"/"Min FPS"/"Max FPS" columns were sampled from `Engine.get_frames_per_second()`, Godot's own smoothed counter — this is the exact metric the next section identifies as insufficient to catch a stuttering frame hidden inside a high rolling average. The "Time to settle" / "Physical simulation time" / "Ticks" columns are unaffected (they don't depend on the FPS counter at all) and remain valid. Kept here, unmodified, as the historical record of what this methodology produced.

---

## Stress Test — Frame-Time / Stutter / Simulation-Tick Instrumentation

**Status: INSTRUMENTATION SHIPPED. Benchmark re-run PENDING MANUAL HUMAN TESTING — no new 10k–500k numbers are recorded in this document by this milestone.** See [SIMULATION_TIMESCALE.md "Stress-Test Instrumentation Trustworthiness"](SIMULATION_TIMESCALE.md#stress-test-instrumentation-trustworthiness-frame-time--stutter--per-tick-timing) for the architectural writeup this section summarizes.

**Problem report:** the table above reported avg FPS of ~2100 (10k) down to ~1127 (500k) — numbers that read as comfortably excellent. Manually playing the 500k tier, however, visibly and repeatedly stutters. A benchmark whose headline number contradicts direct observation is not trustworthy, regardless of how good the number looks.

**Root cause, found by reading the measurement code, not assumed:** two independent gaps in what the old harness measured.

1. **`Engine.get_frames_per_second()` is a smoothed counter, not a per-frame reading.** Godot averages it over roughly the last second internally. Sampling it once per render frame and then averaging *those samples* smooths an already-smoothed number twice — a handful of genuine multi-frame stalls can sit well inside a one-second smoothing window and never move the reported average by much, while still being fully visible to a human as a stutter.
2. **`sim_ms` was being sampled once per render frame from `PixelSimWorld.get_stats()`**, a value that only changes when `step_simulation()` actually runs. After [Stress Test — Fixed-Timestep Physics](#stress-test--fixed-timestep-physics) decoupled the ~60 tick/s physics rate from render FPS, a 1000+ FPS render loop runs a real simulation tick on only a small fraction of its frames — every other frame's `sim_ms` sample was silently rereading the previous tick's stale cached value, which is not a meaningful per-frame or per-tick cost signal once the two rates diverge this far.

**Fix — new instrumentation, zero simulation/gameplay code changed:**

- **`CPUSimulationBackend`** (`project/scripts/cpu_simulation_backend.gd`) now times every `step_simulation()` call directly with `Time.get_ticks_usec()`, immediately around the call — a tick sample always corresponds to a tick that actually ran, never a resample of stale state. `last_advance_tick_usec`/`last_advance_ticks` (sum of the current frame's own tick timings) are always maintained at negligible cost; the full per-tick history (`tick_duration_samples`, needed for percentiles) is gated behind an opt-in `record_tick_samples` flag (default `false`) so normal, non-benchmark gameplay never grows an unbounded array — `stress_test.gd` turns it on only for the duration of a measured tier and off again in `_finish_report()`.
- **`stress_test.gd`** no longer samples `Engine.get_frames_per_second()` at all. FPS is now derived directly from each frame's own raw `delta` (`1.0 / delta`), and the harness reports the full frame-time distribution — average, **median**, p95, p99, max — plus explicit stutter-frame counts (frames slower than 16.67ms / 33.33ms / 50ms, i.e. worse than 60/30/20fps-equivalent). Simulation-tick timing gets the same distribution treatment (avg/median/p95/p99/max), sourced from `CPUSimulationBackend.tick_duration_samples`. A frame-vs-simulation split (`avg frame-sim-time` vs. `avg frame remainder`) is also reported, so a stutter can be attributed to "simulation-bound" or "something else" instead of guessed at.
- The report explicitly states `Simulation Backend: CPU` / `GPU Backend: NOT WIRED INTO GAMEPLAY` on every run (see [Architectural Invariants](#architectural-invariants)) so a GPU PoC number is never confused with what this harness actually measures.
- **Nothing about the primary metric changed.** Time to settle, physical simulation time, tick count, fresh-scene isolation, V-Sync-off/uncapped-FPS benchmark mode, and the settle-detection logic are all byte-for-byte unchanged from [Stress Test — Fixed-Timestep Physics](#stress-test--fixed-timestep-physics) above — this milestone only replaced *what gets measured and reported about* an unchanged run, not how the run itself is triggered or paced.

**Regression check performed (code-level, per this milestone's own no-fabricated-benchmark-numbers instruction):** `CPUFPSIndependenceTest.run()` (`project/scripts/cpu_fps_independence_test.gd`) was re-run live after the above change and still reports byte-exact identical physical state across 60/120/240/500 synthetic render cadences, exactly 60 ticks/sec at every cadence — confirming the new timing instrumentation added no observable overhead or side effect to the accumulator/backend it measures. This is a correctness check, not a stress-tier benchmark run — no 10k–500k tier was triggered by this work.

**What this section deliberately does NOT contain:** a re-measured 10k–500k table. Per the request that triggered this milestone, the actual benchmark re-run — and the direct question of why a high average-FPS number coexisted with visible stutter — is answered from a **human-run, human-observed** pass using the instrumentation above, not generated automatically. See [How to Run the Stress Test Manually](#how-to-run-the-stress-test-manually) below for the exact steps; this document will be updated with the real numbers once that pass is reported back.

### How to Run the Stress Test Manually

1. Play `Main.tscn` from a genuinely fresh editor run (or `stop_scene()`/`play_scene()` if already running — see [Stress Test Isolation](#stress-test-isolation)).
2. Press **Shift+1** for the 10k tier. Wait for `[StressTest]`/`=== STRESS TEST RESULT ===` to print (settle is automatic — no further input needed).
3. Note: Time to settle, Settled/Fresh Scene/V-Sync confirmation, avg/median/min/max FPS, avg/p95/p99/max frame time, the three stutter-frame counts, avg/p95/p99/max simulation tick time, and **your own subjective impression of whether the game visibly stuttered** during that run.
4. Restart the scene (fresh scene required before every tier — see [Stress Test Isolation](#stress-test-isolation)).
5. Repeat for **Shift+2** (50k), **Shift+3** (100k), **Shift+4** (250k), **Shift+5** (500k) — fresh scene before each.
6. Report the printed results and your own observations back for analysis. The frame-time/stutter/simulation-tick numbers exist specifically so a claim like "FPS was high but it stuttered" can be checked objectively against the data rather than argued from the FPS average alone.
